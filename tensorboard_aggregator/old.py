...
def extract(dpath, subpath):
    #     scalar_accumulators = [EventAccumulator(path).Reload(
    #     ).scalars for dname in os.listdir(os.path.join(dpath,dname,subpath)) if dname != FOLDER_NAME]
...
def aggregate(dpath, output, subpaths):
    ...
    Args:
        dpath (Posix path): Root directory
        output (str): Whether output should be to CSV or Tensorboard summary
        subpaths (list, Posix paths): List of all folders with Tensorboard summaries
